# UVG Raycaster — Proyecto de Gráficas por Computadora
...
*(El rubro de "hardware distinto a una computadora tradicional" no se
implementó en esta entrega.)*

## Nota sobre los assets

Las texturas de pared, el sprite del enemigo, el arma, la música y los
efectos de sonido fueron provistos por el estudiante y coordinados con el
profesor del curso.

## Para la entrega

1. Sube este repo a GitHub (ver comandos abajo).
2. Graba un video corto mostrando: movimiento sin atravesar paredes, las
   distintas texturas de pared, rotación con mouse, disparo a un enemigo,
   el minimapa, la música/sonidos, el menú con selección de nivel y la
   pantalla de éxito.
3. Entrega el link al repo + el video.

```bash
git init   # si no lo hiciste ya
git add .
git commit -m "Raycaster - Proyecto Graficas por Computadora"
git branch -M main
